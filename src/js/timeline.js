...
function setupTimeline() {
  ...
  timelineData.maxDate =
    timelineData.sortedEntries[timelineData.sortedEntries.length - 1].date;

  // Update timeline display
  document.getElementById("timelineStart").textContent = formatDate(
    timelineData.minDate
  );
  document.getElementById("timelineEnd").textContent = formatDate(
    timelineData.maxDate
  );
  ...
  if (todayStartIndex >= 0) {
    ...
    const minPercent =
      (todayStartIndex / (timelineData.sortedEntries.length - 1)) * 100;
    const maxPercent =
      (todayEndIndex / (timelineData.sortedEntries.length - 1)) * 100;
...
function updateSliderSelection() {
  ...
  // Update display
  document.getElementById("selectedStart").textContent = formatDate(
    startEntry.date
  );
  document.getElementById("selectedEnd").textContent = formatDate(
    endEntry.date
  );
